...
export function Library() {
  ...
  return (
    <div className="min-h-screen bg-[#121212] text-white px-4 sm:px-8 py-8 pb-28">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between mb-10 gap-4">
          <div>
            ...
            <p className="text-white/50">Tes playlists, tes coups de cœur, ta musique</p>
          </div>
          <div className="flex items-center gap-3">
            {isAuthenticated && (
              <>
                <Link to="/wrapped"
                  className="flex items-center gap-2 px-5 py-3 bg-gradient-to-r from-purple-600 to-indigo-600 text-white font-bold rounded-full hover:scale-105 transition-all shadow-lg border border-white/5">
                  <Trophy size={18} /> Wrapped
                </Link>
                <button onClick={() => setShowImport(true)}
                  className="flex items-center gap-2 px-5 py-3 bg-white/10 text-white font-bold rounded-full hover:bg-white/20 transition-all border border-white/5">
            ...
            <div className="bg-[#282828] rounded-2xl p-6 w-full max-w-md shadow-2xl animate-in zoom-in-95 duration-200" onClick={e => e.stopPropagation()}>
              ...
              <form onSubmit={handleCreate} className="space-y-4">
                <input type="text" placeholder="Nom de la playlist" value={newName} onChange={e => setNewName(e.target.value)}
                  className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white placeholder-white/40 outline-none focus:border-white/30 transition-all font-medium"
                  autoFocus />
                <label className="flex items-center gap-3 cursor-pointer">
                  ...
                  <span className="text-white/70 text-sm">Rendre publique (visible par tous)</span>
                </label>
                <div className="flex gap-3 pt-2">
                  <button type="button" onClick={() => setShowCreate(false)} className="flex-1 py-3 rounded-xl border border-white/10 text-white/60 hover:bg-white/5 transition-colors font-medium">Annuler</button>
            ...
            <div className="bg-[#282828] rounded-2xl p-6 w-full max-w-md shadow-2xl animate-in zoom-in-95 duration-200" onClick={e => e.stopPropagation()}>
              <div className="flex items-center gap-3 mb-2">
                <div className="w-10 h-10 rounded-lg bg-[#1DB954] flex items-center justify-center">
                  <Share2 size={20} className="text-white rotate-180" />
                </div>
                <h2 className="text-xl font-bold">Import Spotify</h2>
              </div>
              <p className="text-white/50 text-sm mb-5">Colle le lien d'une playlist Spotify publique pour l'importer instantanément.</p>
              ...
              <form onSubmit={handleImport} className="space-y-4">
                ...
                <div className="flex gap-3 pt-2">
                  <button type="button" onClick={() => setShowImport(false)} className="flex-1 py-3 rounded-xl border border-white/10 text-white/60 hover:bg-white/5 transition-colors font-medium">Annuler</button>
                  <button type="submit" disabled={importing || !importUrl.trim()} className="flex-1 py-3 rounded-xl bg-[#1DB954] text-white font-bold hover:bg-[#1ed760] transition-colors disabled:opacity-50">
        ...
        {isAuthenticated && (
          <div className="mb-10">
            <h2 className="text-lg font-bold text-white/70 uppercase tracking-widest mb-4 flex items-center gap-2">
            ...
            <Link to={likedPlaylist ? `/library/${likedPlaylist.id}` : '#'}
              ...
              <div className="flex-1 min-w-0">
                <p className="text-white text-xl font-bold">Titres Likés</p>
                <p className="text-white/50 text-sm mt-1">Tous les titres que tu as aimés ❤️</p>
              </div>
        ...
        {isAuthenticated && (
          <div className="mb-12">
            <h2 className="text-lg font-bold text-white/70 uppercase tracking-widest mb-5 flex items-center gap-2">
        ...
        <div>
          ...
          {loadingPublic ? (
            <div className="flex justify-center py-16"><Loader2 size={32} className="animate-spin text-white/30" /></div>
...
function PlaylistCard({ playlist, isOwner, onDelete, onSave, navigate }) {
  ...
  return (
    <div className="relative group">
      <div onClick={() => navigate(`/library/${playlist.id}`)}
        ...
        <div className="aspect-square rounded-lg bg-gradient-to-br from-white/10 to-white/5 flex items-center justify-center overflow-hidden relative">
          ...
          <div className="absolute inset-0 flex items-end justify-between p-2 opacity-0 group-hover:opacity-100 transition-opacity">
            <button onClick={e => { e.stopPropagation(); setShowMenu(v => !v); }}
              className="p-1.5 bg-black/70 rounded-full text-white/80 hover:text-white">
        ...
        <div>
          ...
          <div className="flex items-center gap-1.5 mt-1">
            {playlist.is_public ? <Globe size={11} className="text-white/40" /> : <Lock size={11} className="text-white/40" />}
      ...
      {/* Context Menu */}
      {showMenu && (
        <div className="absolute left-0 bottom-full mb-1 w-44 bg-[#282828] border border-white/10 rounded-xl shadow-2xl z-50 overflow-hidden animate-in fade-in zoom-in-95 duration-150" onClick={e => e.stopPropagation()}>
          {isOwner ? (
            ...
            <button onClick={() => { onSave(playlist.id); setShowMenu(false); }}
              className="w-full flex items-center gap-2.5 px-3 py-2.5 text-sm text-white/80 hover:bg-white/10 transition-colors text-left">
          ...
          <button onClick={() => { navigator.clipboard.writeText(`${window.location.origin}/library/${playlist.id}`); setShowMenu(false); }}
            className="w-full flex items-center gap-2.5 px-3 py-2.5 text-sm text-white/60 hover:bg-white/10 transition-colors text-left border-t border-white/5">
